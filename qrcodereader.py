...
def scaledNumber(newMin, newMax, prevMin, prevMax, toScale):
    """Scales toScale between newMin and newMax.
    ...
    """
    newNumber = (
        ((newMin - newMax) * (float(toScale) - prevMin)) / (prevMax - prevMin)
    ) + newMax
    if newNumber < newMin:
        return newMin
...
# May change how favicons are handled/deleted later.
